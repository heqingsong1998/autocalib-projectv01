...
from drivers.torque_motor.torque_card import TorqueMotorCard  # noqa: E402
...
class MotorControlUI(QMainWindow):
    def __init__(self):
        ...
        self.initUI()
        self.load_conn_defaults()
        self.connect_motor()

    # ---------- UI 结构 ----------
    def initUI(self):
        self.setWindowTitle("电机控制（参考 motioncard 风格）")
        ...
        self.baud_edit.setRange(1200, 10000000)
        self.baud_edit.setValue(115200)
        self.slave_edit = QSpinBox()
        self.slave_edit.setRange(0, 247)
        self.slave_edit.setValue(0)
        self.connect_btn = QPushButton("重连")
        self.connect_btn.clicked.connect(self.reconnect)
        cg.addWidget(QLabel("串口："), 0, 0)
        cg.addWidget(self.port_edit, 0, 1)
        cg.addWidget(QLabel("波特率："), 0, 2)
        cg.addWidget(self.baud_edit, 0, 3)
        cg.addWidget(QLabel("从站："), 0, 4)
        cg.addWidget(self.slave_edit, 0, 5)
        ...
        rg = QGridLayout()
        self.rel_dist = QDoubleSpinBox()
        self.rel_dist.setRange(-1000, 1000)
        self.rel_dist.setValue(10.0)
        self.rel_dist.setSuffix(" mm")
        self.rel_vel = QDoubleSpinBox()
        self.rel_vel.setRange(0.1, 1000)
        self.rel_vel.setValue(10.0)
        self.rel_vel.setSuffix(" mm/s")
        self.rel_acc = QDoubleSpinBox()
        self.rel_acc.setRange(1.0, 5000)
        self.rel_acc.setValue(50.0)
        self.rel_acc.setSuffix(" mm/s²")
        self.rel_dec = QDoubleSpinBox()
        self.rel_dec.setRange(1.0, 5000)
        self.rel_dec.setValue(50.0)
        self.rel_dec.setSuffix(" mm/s²")
        self.rel_band = QDoubleSpinBox()
        self.rel_band.setRange(0.001, 10.0)
        self.rel_band.setValue(0.1)
        self.rel_band.setSuffix(" mm")
        self.rel_btn = QPushButton("执行相对运动")
        self.rel_btn.clicked.connect(self.on_move_rel)

        rg.addWidget(QLabel("距离 Δ (mm):"), 0, 0)
        rg.addWidget(self.rel_dist, 0, 1)
        rg.addWidget(QLabel("速度 v (mm/s):"), 0, 2)
        rg.addWidget(self.rel_vel, 0, 3)
        rg.addWidget(QLabel("加速度 acc (mm/s²):"), 1, 0)
        rg.addWidget(self.rel_acc, 1, 1)
        rg.addWidget(QLabel("减速度 dec (mm/s²):"), 1, 2)
        rg.addWidget(self.rel_dec, 1, 3)
        rg.addWidget(QLabel("定位带宽 band (mm):"), 2, 0)
        rg.addWidget(self.rel_band, 2, 1)
        ...
        ag = QGridLayout()
        self.abs_pos = QDoubleSpinBox()
        self.abs_pos.setRange(-10000, 10000)
        self.abs_pos.setValue(50.0)
        self.abs_pos.setSuffix(" mm")
        self.abs_vel = QDoubleSpinBox()
        self.abs_vel.setRange(0.1, 1000)
        self.abs_vel.setValue(10.0)
        self.abs_vel.setSuffix(" mm/s")
        self.abs_acc = QDoubleSpinBox()
        self.abs_acc.setRange(1.0, 5000)
        self.abs_acc.setValue(50.0)
        self.abs_acc.setSuffix(" mm/s²")
        self.abs_dec = QDoubleSpinBox()
        self.abs_dec.setRange(1.0, 5000)
        self.abs_dec.setValue(50.0)
        self.abs_dec.setSuffix(" mm/s²")
        self.abs_band = QDoubleSpinBox()
        self.abs_band.setRange(0.001, 10.0)
        self.abs_band.setValue(0.1)
        self.abs_band.setSuffix(" mm")
        self.abs_btn = QPushButton("执行绝对运动")
        self.abs_btn.clicked.connect(self.on_move_abs)

        ag.addWidget(QLabel("目标位置 (mm):"), 0, 0)
        ag.addWidget(self.abs_pos, 0, 1)
        ag.addWidget(QLabel("速度 v (mm/s):"), 0, 2)
        ag.addWidget(self.abs_vel, 0, 3)
        ag.addWidget(QLabel("加速度 acc (mm/s²):"), 1, 0)
        ag.addWidget(self.abs_acc, 1, 1)
        ag.addWidget(QLabel("减速度 dec (mm/s²):"), 1, 2)
        ag.addWidget(self.abs_dec, 1, 3)
        ag.addWidget(QLabel("定位带宽 band (mm):"), 2, 0)
        ag.addWidget(self.abs_band, 2, 1)
        ...
        pg = QGridLayout()
        self.push_force = QDoubleSpinBox()
        self.push_force.setRange(0.1, 200.0)
        self.push_force.setValue(5.0)
        self.push_force.setSuffix(" N")
        self.push_dist = QDoubleSpinBox()
        self.push_dist.setRange(0.1, 1000.0)
        self.push_dist.setValue(10.0)
        self.push_dist.setSuffix(" mm")
        self.push_vel = QDoubleSpinBox()
        ...
        pg.addWidget(QLabel("力阈值 F (N):"), 0, 0)
        pg.addWidget(self.push_force, 0, 1)
        pg.addWidget(QLabel("最大行程 D (mm):"), 0, 2)
        pg.addWidget(self.push_dist, 0, 3)
        pg.addWidget(QLabel("速度 v (mm/s):"), 1, 0)
        pg.addWidget(self.push_vel, 1, 1)
        ...
        pg2 = QGridLayout()
        self.pp_force = QDoubleSpinBox()
        self.pp_force.setRange(0.1, 200.0)
        self.pp_force.setValue(5.0)
        self.pp_force.setSuffix(" N")
        self.pp_dist = QDoubleSpinBox()
        self.pp_dist.setRange(0.1, 1000.0)
        self.pp_dist.setValue(10.0)
        self.pp_dist.setSuffix(" mm")
        self.pp_vel = QDoubleSpinBox()
        self.pp_vel.setRange(0.1, 100.0)
        self.pp_vel.setValue(2.0)
        self.pp_vel.setSuffix(" mm/s")
        self.pp_band = QDoubleSpinBox()
        self.pp_band.setRange(0.01, 10.0)
        self.pp_band.setValue(0.2)
        self.pp_band.setSuffix(" N")
        self.pp_chkms = QSpinBox()
        self.pp_chkms.setRange(10, 5000)
        self.pp_chkms.setValue(200)
        self.pp_btn = QPushButton("进入闭环推压（需 STOP 退出）")
        self.pp_btn.clicked.connect(self.on_precise_push)

        pg2.addWidget(QLabel("目标力 F (N):"), 0, 0)
        pg2.addWidget(self.pp_force, 0, 1)
        pg2.addWidget(QLabel("最大行程 D (mm):"), 0, 2)
        pg2.addWidget(self.pp_dist, 0, 3)
        pg2.addWidget(QLabel("速度 v (mm/s):"), 1, 0)
        pg2.addWidget(self.pp_vel, 1, 1)
        pg2.addWidget(QLabel("力带宽 (N):"), 1, 2)
        pg2.addWidget(self.pp_band, 1, 3)
        pg2.addWidget(QLabel("判稳时间 (ms):"), 2, 0)
        pg2.addWidget(self.pp_chkms, 2, 1)
        ...
        # 操作行：回零/STOP/力清零
        op_line = QHBoxLayout()
        self.home_btn = QPushButton("回原点")
        self.home_btn.clicked.connect(self.on_home)
        self.stop_btn = QPushButton("STOP（最高优先）")
        self.stop_btn.setStyleSheet("background:#d32f2f;color:white;font-weight:bold;")
        self.stop_btn.clicked.connect(self.on_stop)
        self.zero_btn = QPushButton("力清零 (#25)")
        self.zero_btn.clicked.connect(self.on_force_zero)
        for b in (self.home_btn, self.stop_btn, self.zero_btn):
            op_line.addWidget(b)
    ...
    # ---------- 连接与状态 ----------
    def load_conn_defaults(self):
        """从 default.yaml 中读取 torque_motor 端口/波特率/从站并填充到输入框。"""
        try:
            with open(CONFIG_PATH, "r", encoding="utf-8") as f:
                cfg = yaml.safe_load(f) or {}
            tm_cfg = cfg.get("torque_motor") or {}
            port = tm_cfg.get("port")
            baud = tm_cfg.get("baud")
            slave = tm_cfg.get("slave")
            if port:
                self.port_edit.setText(str(port))
            if baud is not None:
                self.baud_edit.setValue(int(baud))
            if slave is not None:
                self.slave_edit.setValue(int(slave))
            self.log(f"已从配置加载端口/波特率: port={port}, baud={baud}, slave={slave}")
        except FileNotFoundError:
            self.log(f"未找到配置文件：{CONFIG_PATH}")
        except Exception as e:
            self.log(f"读取配置失败：{e}")
    ...
    def connect_motor(self):
        try:
            ...
            try:
                ...
                self.log(
                    f"已连接。版本 {getattr(v, 'major', '?')}.{getattr(v, 'minor', '?')}.{getattr(v, 'build', '?')} (type={getattr(v, 'type', '?')})"
                )
            except Exception:
                self.log("已连接。版本读取失败")
    ...
    def on_precise_push(self):
        ...
        def job():
            ...
            self.log("提示：闭环推压是持续力控模式，请用 STOP 退出再进行其他运动。")
    ...
    def on_stop(self):
        ...
        self.log("STOP 发出。" + (" 已停止并释放" if ok else " 可能未完全生效"))
