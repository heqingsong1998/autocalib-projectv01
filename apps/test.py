...
# 在 PlotWindow 类中修改 update_plot 方法
...
class PlotWindow(QMainWindow):
    ...
    def update_plot(self):
        ...
        if wuzhou_data:
            ...
            data_mapping = {
                "wuzhou_fz": (5, 100.0*(-1)),  # FZ 对应 data[5]，缩小 100 倍
...
class MainWindow(QMainWindow):
    ...
    def create_sensor_displays(self, main_layout):
        ...
        # 五轴显示
        five_group = QGroupBox("五轴最新值 (c1 c2 c3 c4 c0 FZ MY MX FX FY)")
    ...
    def on_snapshot(self):
        ...
        try:
            window_s = 1.0
            t_end = RingBuffer.now_mono()
            t_beg = t_end - window_s
            ...
            if a_frames == 0 and b_frames == 0:
                self.log_print("⚠️ 最近 1.0s 内无数据，未写入")
                return
            ...
            try:
                wb = load_workbook(self.xlsx_path)
                ws = wb["Sheet2_Aligned"]
                snapshot_id = ws.max_row if ws.max_row > 1 else 0
            except Exception:
                snapshot_id = 0

            values = [snapshot_id + 1, gen_time, window_s, a_frames, b_frames] + a_mean + b_mean
            self.raw_writer.append_sheet2_row(header, values)
            self.log_print(f"✅ 对齐完成：#{snapshot_id + 1} 窗口={window_s}s six_frames={a_frames} five_frames={b_frames}")
        except Exception as e:
            self.log_print(f"❌ 对齐失败：{e}")
        ...
        with self.mgr._lock:
            if self.mgr.latest["liuzhou"]:
                ...
                self.six_label.setText(
                    f"#{frame:05d}  Fx={Fx:8.4f}  Fy={Fy:8.4f}  Fz={Fz:8.4f}  "
