...
# 注意：这里的导入路径已经从 wuzhou 改为 liuzhouwy
from drivers.liuzhouwy.utils import create_sensor, initialize_sensor, display_sensor_data
...
def main():
    ...
    try:
        # 加载配置并创建传感器
        config = load_config()
        # 注意：配置节已改为 lab_six_axis
        sensor_config = config["sensor"]["lab_six_axis"]
