...
class TorqueMotorCard(TorqueBaseCard):
    ...
    def _axis_call(self, name: str, *args):
        with self._io_lock:
            _ck(self.axis is not None, name)
            fn = getattr(self.axis, name)
            return fn(*args)
        ...
        with self._io_lock:
            ...
            try:
                try:
                    ...
                    pass
                self.axis.set_servo_on_off(True)
                self.connected = True
                return 0
    ...
    def disconnect(self) -> int:
        with self._io_lock:
            if not self.axis:
                ...
                return 0
            try:
                self.axis.set_servo_on_off(False)
            except Exception:
                pass
            try:
                motormaster.destroy_axis(self.axis)
    ...
    def set_servo(self, on: bool) -> int:
        self._axis_call("set_servo_on_off", bool(on))
        return 0
    ...
    def config_home(self, velocity: float, acceleration: float, deacceleration: float) -> int:
        ...
        self._axis_call("config_motion", float(velocity), float(acceleration), float(deacceleration))
        return 0


    # ============== 运动控制 ==============
    def move_abs(self, axis: int, pos: float) -> int:
        self._axis_call("move_absolute",
                        float(pos),
    ...
    def stop(self, axis: int, mode: int = 0) -> int:
        self._axis_call("stop")
        return 0

    def is_done(self, axis: int) -> bool:
        try:
            return not bool(self._axis_call("is_moving"))
        except Exception:
            return False

    def get_position(self, axis: int) -> float:
        return float(self._axis_call("position"))

    def get_velocity(self, axis: int) -> float:
        return float(self._axis_call("velocity"))

    def get_force(self) -> float:
        return float(self._axis_call("force_sensor"))

    # ============== 回零/位置 ==============
    def home(self, axis: int) -> int:
        self._axis_call("go_home")
        return 0
    ...
    def push(self, force_n: float, dist_mm: float, vel_mm_s: float) -> int:
        self._axis_call("push", float(force_n), float(dist_mm), float(vel_mm_s))
        return 0

    def precise_push(self, force_n: float, dist_mm: float, vel_mm_s: float, band_n: float, chk_ms: int) -> int:
        self._axis_call("precise_push",
                        float(force_n), float(dist_mm), float(vel_mm_s),
                        float(band_n), int(chk_ms))
        return 0

    def trigger_command(self, code: int) -> int:
        # 底层真实函数名：trig_command
        self._axis_call("trig_command", int(code))
        return 0

    # ============== 可选 ==============
    def get_version(self) -> Any:
        return self._axis_call("get_version")
